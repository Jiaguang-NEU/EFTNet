...
class Compose(object):
    ...
    def __call__(self, image, label, label2):
        for t in self.segtransform:
            if self.segtransform.index(t) == 0:
                image, label, label2, black_record = t(image, label, label2)
            else:
                image, label, label2 = t(image, label, label2)
        return image, label, label2, black_record
...
class Resize(object):
    ...
    def __call__(self, image, label, label2):
        ...
        black = np.copy(back_crop_s_mask)
        ...
        return image, label, label2, black
